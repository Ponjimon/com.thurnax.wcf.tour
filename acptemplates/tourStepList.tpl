...
{if $tourID}
	...
	<div class="contentNavigation">
		...
		<nav>
			<ul>
				<li class="button jsTourRestart" data-object-id="{$tourID}">
					<span class="icon icon16 icon-play"></span>
					<span>{lang}wcf.acp.tour.restartTour{/lang}</span>
				</li>
				<li id="jumpToTourTop" class="dropdown">
					<a class="button dropdownToggle" data-toggle="jumpToTourTop">
						<span class="icon icon16 icon-sort"></span>
						<span>{lang}wcf.acp.tour.step.jumpToTour{/lang}</span>
					</a>
				...
				<li>
					<a href="{link controller='TourStepAdd' object=$tours[$tourID]}{/link}" class="button">
			...
			<table data-type="com.thurnax.wcf.tour.step" class="table jsClipboardContainer">
				<thead>
					<tr>
						...
						<th class="columnDigits{if $sortField == 'showOrder'} active {@$sortOrder}{/if}" colspan="2">
							<a href="{link controller='TourStepList'}pageNo={@$pageNo}&sortField=showOrder&sortOrder={if $sortField == 'showOrder' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.acp.tour.step.showOrder{/lang}</a>
						</th>
						<th class="columnText{if $sortField == 'target'} active {@$sortOrder}{/if}">
							<a href="{link controller='TourStepList'}pageNo={@$pageNo}&sortField=target&sortOrder={if $sortField == 'target' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.acp.tour.step.target{/lang}</a>
						</th>
						<th class="columnText{if $sortField == 'title'} active {@$sortOrder}{/if}">
							<a href="{link controller='TourStepList'}pageNo={@$pageNo}&sortField=title&sortOrder={if $sortField == 'title' && $sortOder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.acp.tour.step.title{/lang}</a>
						</th>
						<th class="columnText{if $sortField == 'content'} active {@$sortOrder}{/if}">
							<a href="{link controller='TourStepList'}pageNo={@$pageNo}&sortField=content&sortOrder={if $sortField == 'content' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.acp.tour.step.content{/lang}</a>
						</th>
						<th class="columnText{if $sortField == 'placement'} active {@$sortOrder}{/if}">
							<a href="{link controller='TourStepList'}pageNo={@$pageNo}&sortField=placement&sortOrder={if $sortField == 'placement' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.acp.tour.step.placement{/lang}</a>
						</th>
				...
				<tbody class="sortableList simpleSortableList" data-object-id="0">
					{foreach from=$objects item=tourStep}
						<tr class="jsTourStepRow sortableNode" data-object-id="{@$tourStep->tourStepID}">
							...
							<td class="columnIcon">
								<span class="icon icon16 icon-check{if $tourStep->isDisabled}-empty{/if} jsToggleButton jsTooltip pointer"
								      title="{lang}wcf.global.button.{if $tourStep->isDisabled}enable{else}disable{/if}{/lang}"
								      data-object-id="{$tourStep->tourStepID}" data-disable-message="{lang}wcf.global.button.disable{/lang}"
								      data-enable-message="{lang}wcf.global.button.enable{/lang}"></span>
								<a href="{link controller='TourStepEdit' id=$tourStep->tourStepID}{/link}" title="{lang}wcf.global.button.edit{/lang}"
								   class="jsTooltip"><span class="icon icon16 icon-pencil"></span>
								</a>
								<span class="icon icon16 icon-remove jsDeleteButton jsTooltip pointer" title="{lang}wcf.global.button.delete{/lang}"
								      data-object-id="{@$tourStep->tourStepID}"
								      data-confirm-message="{lang}wcf.acp.tour.step.delete.sure{/lang}"></span>
		...
		<div class="contentNavigation">
			...
			<nav>
				<ul>
					...
					<li id="jumpToTourBottom" class="dropdown">
						<a class="button dropdownToggle" data-toggle="jumpToTourBottom">
							<span class="icon icon16 icon-sort"></span>
							<span>{lang}wcf.acp.tour.step.jumpToTour{/lang}</span>
						</a>
						<ul class="dropdownMenu">
...
{else}
	<div class="contentNavigation">
		<nav>
			<ul>
				<li><a href="{link controller='TourStepAdd'}{/link}" class="button"><span class="icon icon16 icon-plus"></span>
						<span>{lang}wcf.acp.tour.step.add{/lang}</span></a></li>
