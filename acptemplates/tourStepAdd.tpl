...
<div class="contentNavigation">
	<nav>
		<ul>
			<li>
				<a href="{if $tourID}{link controller='TourStepList' object=$tours[$tourID]}{/link}{else}{link controller='TourStepList' object=$tours|current}{/link}{/if}"
				   class="button">
					<span class="icon icon16 icon-list"></span>
					<span>{lang}wcf.acp.tour.step.list{/lang}</span>
				</a>
			</li>
...
<form method="post"
      action="{if $action == 'add'}{link controller='TourStepAdd'}{/link}{else}{link controller='TourStepEdit' id=$tourStepID}{/link}{/if}">
			...
			<fieldset>
				...
				<dl{if $errorField == 'title'} class="formError"{/if}>
					...
					<dd>
						...
						{if $errorField == 'title'}
							<small class="innerError">{lang}wcf.global.form.error.{@$errorType}{/lang}</small>{/if}
				...
				<dl{if $errorField == 'stepContent'} class="formError"{/if}>
					...
					<dd>
						<textarea id="stepContent" name="stepContent" required="required" cols="40"
						          rows="10">{$i18nPlainValues['stepContent']}</textarea>
						{if $errorField == 'stepContent'}
							<small class="innerError">{lang}wcf.global.form.error.{@$errorType}{/lang}</small>{/if}
					</dd>
				</dl>
			...
			<fieldset>
				...
				<dl{if $errorField == 'eitherUrlOrOnNext'} class="formError"{/if}>
					...
					<dd>
						...
						{if $errorField == 'eitherUrlOrOnNext'}
							<small class="innerError">{lang}wcf.acp.tour.step.eitherUrlOrOnNext{/lang}</small>{/if}
